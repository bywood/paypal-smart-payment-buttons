...
import { BUTTON_RENDER_JS, BUTTON_CLIENT_JS, SMART_BUTTONS_MODULE, CHECKOUT_COMPONENTS_MODULE,
    BUTTON_CLIENT_MIN_JS, WEBPACK_CONFIG, ACTIVE_TAG, SDK_CDN_NAMESPACE } from '../../config';
...
export async function getPayPalSmartPaymentButtonsRenderScript({ logBuffer, cache, useLocal = isLocalOrTest(), cdnHostName, sdkCDNRegistry, sdkVersionManager } : GetPayPalSmartPaymentButtonsRenderScriptOptions) : Promise<SmartPaymentButtonsRenderScript> {
    if (useLocal) {
    ...
    const moduleDetails = await sdkVersionManager.getOrInstallSDK({
        cdnRegistry:  sdkCDNRegistry || `https://${ cdnHostName }/${ SDK_CDN_NAMESPACE }`,
